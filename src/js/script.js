...
const init = function() {
    ...
    fileInput.addEventListener('change', async () => {
        ...
        await parseDOCX(selectedFile);
    });
}

const parseDOCX = async function(file) {
    ...
    const documentContent = parseDOM(XMLDocument);
    // console.log(documentContent);
}

const parseDOM = function(XMLDocument) {
    ...
    const documentContent = Array.from(XMLDocument.querySelector('document').querySelector('body').childNodes);
    console.log(documentContent);

    // Проходимся по узлам контента документа и выбираем то, что необходимо
    const newResult = documentContent.map((node) => {
        return parseNodeContent(node, resultContent);
    });

    console.log(newResult);
}

const parseNodeContent = function(node, resultContent = null) {
